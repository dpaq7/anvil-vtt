...
export function MobileCampaigns() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api
      .get<{ campaigns: CampaignData[] }>('/api/game-sessions')
      .then((data) => {
        if (!cancelled) {
  ...
  return (
    <div className={mobileContainerClass()}>
      <div>
        ...
        <p className="mt-1 text-xs text-zinc-500">Read-only structure</p>
      </div>

      {loading ? (
        <LoadingPanel />
      ) : campaigns.length === 0 ? (
        <EmptyState icon={FolderKanban} title="No directed campaigns" />
      ) : (
        <div className="grid gap-4">
          {campaigns.map((campaign) => (
            <CampaignStructureCard key={campaign.id} campaign={campaign} />
          ))}
        </div>
      )}
    </div>
...
function CampaignStructureCard({ campaign }: { campaign: CampaignData }) {
...
function SessionSceneList({
  ...
  if (sessions.length === 0) {
    return <p className="px-3 py-3 text-xs text-zinc-500">No sessions</p>;
  }
