...
import { AppShell, Button, Tabs, TabsContent, TabsList, TabsTrigger } from '@anvil/ui';
import type { SceneType } from '@anvil/ui';
import type { SessionState, AbilityResult, MontageLiveState, NegotiationLiveState } from '../../types/protocol.js';
...
import type { MontageStageData, NegotiationStageData } from '../../lib/scene-data.js';
...
function clampGridCellSize(value: number): number {
  ...
  return Math.max(GRID_CELL_SIZE_MIN, Math.min(GRID_CELL_SIZE_MAX, Math.round(value)));
}
...
export function DirectorView({ sessionState, connectionStatus, send, combatLog }: DirectorViewProps) {
  ...
  const [focusEntityRequest, setFocusEntityRequest] = useState<{ entityId: string; nonce: number } | null>(null);
  const [montageRoundsByScene, setMontageRoundsByScene] = useState<Record<string, boolean[]>>({});
  const [montageTrackingByScene, setMontageTrackingByScene] = useState<Record<string, string[]>>({});
  ...
  const [tokenScaleOverride, setTokenScaleOverride] = useState<number | null>(null);
  const [gridSquareSizeOverride, setGridSquareSizeOverride] = useState<number | null>(null);
  const [gridOffsetXOverride, setGridOffsetXOverride] = useState<number | null>(null);
  const [gridOffsetYOverride, setGridOffsetYOverride] = useState<number | null>(null);

  const battleData = sceneType === 'battle' ? parseBattleData(sceneData) : null;
  const montageData = sceneType === 'montage' ? parseMontageData(sceneData) : null;
  const negotiationData = sceneType === 'negotiation' ? parseNegotiationData(sceneData) : null;
  ...
  const currentTokenScale = clampGridCellSize(tokenScaleOverride ?? battleData?.cellSize ?? 48);
  const currentGridSquareSize = clampGridCellSize(gridSquareSizeOverride ?? battleData?.cellSize ?? 48);
  const currentGridOffsetX = clampGridOffset(gridOffsetXOverride ?? battleData?.gridOffsetX ?? 0, currentGridSquareSize);
  const currentGridOffsetY = clampGridOffset(gridOffsetYOverride ?? battleData?.gridOffsetY ?? 0, currentGridSquareSize);
  ...
  const creatureCount = useMemo(
    () => entities.filter((entity) => entity.type === 'monster' || entity.type === 'npc').length,
    [entities],
  );
  ...
  const activeMontageTracking = activeSceneId ? montageTrackingByScene[activeSceneId] ?? [] : [];
  const activeMontageRounds = activeSceneId ? montageRoundsByScene[activeSceneId] ?? [] : [];
  const leftRailTabs = useMemo<Array<{ value: LeftRailTab; label: string; count?: number }>>(() => {
    const tabs: Array<{ value: LeftRailTab; label: string; count?: number }> = [];
    if (sceneType === 'battle' && combat) tabs.push({ value: 'combat', label: 'Combat' });
    if (sceneType === 'negotiation') {
      ...
      tabs.push({ value: 'tracking', label: 'Tracking', count: activeMontageTracking.length });
    } else {
      tabs.push({ value: 'creatures', label: 'Creatures', count: creatureCount });
    }
  ...
  const handleToggleMontageRound = useCallback((roundIndex: number) => {
    if (!activeSceneId) return;
  ...
  const handleToggleMontageActor = useCallback((actorId: string) => {
    if (!activeSceneId) return;
  ...
  const handleGridSquareSizeChange = useCallback((value: number) => {
    ...
    setGridOffsetXOverride((current) => current === null ? null : clampGridOffset(current, nextSize));
    setGridOffsetYOverride((current) => current === null ? null : clampGridOffset(current, nextSize));
  }, []);
    ...
    <AppShell
        ...
        >
          <TabsList className={RAIL_TABS_LIST_CLASS}>
            {leftRailTabs.map((tab) => (
              <TabsTrigger key={tab.value} value={tab.value} className={RAIL_TAB_TRIGGER_CLASS}>
          ...
          {sceneType === 'battle' && combat && (
            <TabsContent value="combat" className={`${RAIL_TAB_CONTENT_CLASS} overflow-y-auto p-3`}>
              <CombatTracker
                combat={combat}
                entities={entities}
                selectedEntityIds={selectedEntityIds}
          ...
          <TabsContent value="creatures" className={RAIL_TAB_CONTENT_CLASS}>
            {sceneType === 'negotiation' && negotiationData ? (
              <NegotiationTargetPanel data={negotiationData} live={sessionState.negotiation} />
            ) : sceneType === 'montage' && montageData ? (
              <MontageResultsPanel data={montageData} live={sessionState.montage} />
            ) : (
              <CreatureTracker
                entities={entities}
                combat={combat}
      ...
      rightRail={focusMode ? undefined : (
        ...
        >
          ...
          {sceneType && (
            <TabsContent value="assets" className={`${RAIL_TAB_CONTENT_CLASS} overflow-y-auto p-2`}>
              <AssetPanel
                sceneType={sceneType}
          ...
          {sceneType === 'battle' && (
            <TabsContent value="combat" className={`${RAIL_TAB_CONTENT_CLASS} overflow-y-auto p-4`}>
              {!combat ? (
          ...
          {sceneType === 'battle' && battleData && (
            <TabsContent value="grid" className={`${RAIL_TAB_CONTENT_CLASS} overflow-y-auto p-4`}>
              <div className="flex flex-col gap-3">
                <span className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Grid Overlay</span>
                <div className="flex flex-col gap-3">
                  <div className="flex items-center gap-2">
                    <span className="w-20 text-xs text-zinc-500">Opacity</span>
                    <input
                      type="range"
                      min={0}
                      max={100}
                  ...
                  <div className="flex items-center gap-2">
                    <span className="w-20 text-xs text-zinc-500">Token scale</span>
                    <input
                      type="range"
                      ...
                      onChange={(e) => handleTokenScaleChange(Number(e.target.value))}
                      className="h-1 flex-1 accent-zinc-400"
                    />
                    <input
                      type="number"
                  ...
                  <div className="flex items-center gap-2">
                    <span className="w-20 text-xs text-zinc-500">Grid size</span>
                    <input
                      type="range"
                      min={GRID_CELL_SIZE_MIN}
                      max={GRID_CELL_SIZE_MAX}
                      ...
                      onChange={(e) => handleGridSquareSizeChange(Number(e.target.value))}
                      className="h-1 flex-1 accent-zinc-400"
                    />
                    <input
                      type="number"
                  ...
                  <div className="flex items-center gap-2">
                    <span className="w-20 text-xs text-zinc-500">Offset X</span>
                    <input
                      type="range"
                      ...
                      onChange={(e) => setGridOffsetXOverride(Number(e.target.value))}
                      className="h-1 flex-1 accent-zinc-400"
                    />
                    <span className="w-10 text-right text-xs text-zinc-500">{Math.round(currentGridOffsetX)}px</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="w-20 text-xs text-zinc-500">Offset Y</span>
                    <input
                      type="range"
                      min={-currentGridSquareSize}
                      max={currentGridSquareSize}
                      ...
                      onChange={(e) => setGridOffsetYOverride(Number(e.target.value))}
                      className="h-1 flex-1 accent-zinc-400"
                    />
                    <span className="w-10 text-right text-xs text-zinc-500">{Math.round(currentGridOffsetY)}px</span>
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    ...
                    {(gridSquareSizeOverride !== null || gridOffsetXOverride !== null || gridOffsetYOverride !== null) && (
                      <Button
                        type="button"
                        variant="ghost"
                        size="sm"
    ...
    <div className="flex h-full min-h-0 flex-col overflow-y-auto p-3">
      <section className="rounded border border-zinc-800 bg-zinc-900/60 p-3">
        <div className="flex items-start gap-3">
          {data.npcPortrait ? (
            <img src={data.npcPortrait} alt="" className="size-14 rounded object-cover" />
          ) : (
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-semibold text-zinc-100">{data.npcName}</p>
            <p className="mt-1 text-xs capitalize text-zinc-400">{data.npcAttitude}</p>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        ...
        <MetricCard label="Patience" value={`${patience} / ${maxPatience}`} tone="amber" />
      </div>

      {currentOutcome && (
        <section className="mt-3 rounded border border-zinc-800 bg-zinc-900/60 p-3">
          <p className="text-[10px] font-semibold uppercase tracking-wide text-zinc-500">Current outcome</p>
          <p className="mt-1 text-xs leading-relaxed text-zinc-300">{currentOutcome}</p>
        </section>
      )}
...
function MontageResultsPanel({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-y-auto p-3">
      <section className="rounded border border-zinc-800 bg-zinc-900/60 p-3">
        <p className="text-[10px] font-semibold uppercase tracking-wide text-zinc-500">Montage goal</p>
        <p className="mt-1 text-sm font-semibold leading-snug text-zinc-100">{data.goal || 'No goal set'}</p>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        <MetricCard label="Successes" value={`${successes} / ${successLimit}`} tone="emerald" />
        <MetricCard label="Failures" value={`${failures} / ${failureLimit}`} tone="red" />
      </div>

      <OutcomeCard title="Total Success" text={data.totalSuccess} className="border-emerald-500/30" />
      <OutcomeCard title="Partial Success" text={data.partialSuccess} className="border-amber-500/30" />
      <OutcomeCard title="Total Failure" text={data.totalFailure} className="border-red-500/30" />
    </div>
  );
}
    ...
    <div className="flex h-full min-h-0 flex-col overflow-y-auto p-3">
      <section className="rounded border border-zinc-800 bg-zinc-900/60">
        <div className="border-b border-zinc-800 px-3 py-2">
          <p className="text-xs font-semibold uppercase tracking-wide text-zinc-500">Tracking</p>
        </div>
        <div className="flex flex-col p-2">
          {actors.length === 0 ? (
            <p className="px-2 py-4 text-xs text-zinc-600">No players or heroes found.</p>
          ) : (
            actors.map((actor) => (
              <label
                key={actor.id}
...
function SecretList({
  ...
  title: string;
  items: Array<{ id: string; type: string; description: string; revealed: boolean }>;
  tone: 'purple' | 'red';
}) {
        ...
        {items.length === 0 ? (
          ...
          items.map((item) => (
            <div key={item.id} className="rounded bg-zinc-950/50 px-2 py-1.5">
              <p className="text-[10px] uppercase tracking-wide text-zinc-500">{item.type}</p>
...
function OutcomeCard({
  ...
  return (
    <section className={`mt-3 rounded border bg-zinc-900/60 p-3 ${className}`}>
      ...
      <p className="mt-1 text-xs leading-relaxed text-zinc-300">{text || 'No outcome set.'}</p>
    </section>
  );
}
