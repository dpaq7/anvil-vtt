...
export function CharacteristicsStep({ character, onChange }: Props) {
  ...
  // Phone: one screen — array choice plus a full-width chip row per
  // characteristic, driven by the same chooseArray/assignValue handlers
  // desktop uses (including the swap when a value is already taken).
  const remainingList = rules
  ...
  const screens = buildCharacteristicsScreens({
    hasClass: rules !== null,
    helper: selectedArray
      ? `Distribute ${formatArray(selectedArray)} among ${remainingList}.`
      : fixedSummary
        ? `You start with ${fixedSummary}. Pick an array for ${remainingList}.`
        : `Pick an array for ${remainingList}.`,
    arrays: (rules?.arrays ?? []).map((array) => ({
      key: arrayKey(array),
      label: formatArray(array),
      selected: selectedArray ? sameNumberMultiset(selectedArray, array) : false,
      onChoose: () => chooseArray(array),
    })),
    rows: CHARACTERISTIC_NAMES.map((name) => {
      const fixedValue = rules?.fixed[name];
      const isFixed = typeof fixedValue === "number";
      const currentValue = isFixed ? fixedValue : character.characteristics?.[name];
      return {
        id: name,
        name: formatCharacteristicName(name),
        locked: isFixed,
        value: currentValue ?? null,
        chips:
          isFixed || !selectedArray || !character.characteristics
            ? []
            : selectedArrayValues.map((value) => ({
                value,
                selected: character.characteristics?.[name] === value,
                onSelect: () => assignValue(name, value),
              })),
      };
    }),
    isComplete,
    onReset: character.characteristics ? resetArray : null,
  });
