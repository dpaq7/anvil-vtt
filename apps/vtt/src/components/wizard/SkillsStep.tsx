...
import { GameData, WizardLogic, skills as allSkillsData, skillGroups, getSkillsByGroup } from '@anvil/data';
import type { CharacterInProgress, SkillGroup, Skill, CultureBenefit } from '@anvil/data';
import { Card, CardContent, cn, ScrollArea } from '@anvil/ui';
...
function getAvailableSkillsForSource(skillStrings: string[] | undefined): Skill[] {
  ...
  for (const str of skillStrings) {
    ...
    if (['crafting', 'exploration', 'interpersonal', 'intrigue', 'lore'].includes(normalized)) {
...
export function SkillsStep({ character, onChange }: Props) {
  ...
  const skillSources = useMemo(() => {
    ...
    const getSkillGroupsFromBenefit = (benefit: CultureBenefit): string[] => {
      // CultureBenefit has skill?: string (singular, optional)
      // It could be a skill group name like "Exploration" or a specific skill
      return benefit.skill ? [benefit.skill] : [];
    };

    // Environment source
    if (character.culture.environment) {
      const env = environments.find((e) => e.id === character.culture.environment);
      if (env) {
        sources.push({
          id: 'environment',
    ...
    if (character.culture.organization) {
      const org = organizations.find((o) => o.id === character.culture.organization);
      if (org) {
        sources.push({
          id: 'organization',
    ...
    if (character.culture.upbringing) {
      const upb = upbringings.find((u) => u.id === character.culture.upbringing);
      if (upb) {
        sources.push({
          id: 'upbringing',
    ...
    // Career source - show granted skills
    if (character.career) {
      const career = GameData.getCareer(character.career);
      if (career) {
        // Granted skills (no slash)
        const grantedCareerSkills = career.skills.filter((s) => !s.includes('/'));
        ...
        if (grantedCareerSkills.length > 0) {
          sources.push({
            ...
            selectedSkill: grantedCareerSkills[0] ?? null, // Mark first as "selected" (granted)
            isGranted: true,
          });
        }

        // Choice skills (with slash like "Crafting/Exploration")
        ...
        for (let i = 0; i < choiceSkills.length; i++) {
          ...
          sources.push({
            ...
            description: `Choose 1 skill from: ${options.join(' or ')}`,
            skillGroups: options,
    ...
    return sources;
  }, [character.culture, character.career, character.cultureSkills, character.careerSkillChoices]);

  const handleSelectSkill = (sourceId: string, skillName: string) => {
    if (sourceId === 'environment') {
      onChange({
        cultureSkills: { ...character.cultureSkills, environment: skillName },
      });
    } else if (sourceId === 'organization') {
      onChange({
        cultureSkills: { ...character.cultureSkills, organization: skillName },
      });
    } else if (sourceId === 'upbringing') {
      onChange({
        cultureSkills: { ...character.cultureSkills, upbringing: skillName },
      });
    } else if (sourceId.startsWith('career-choice-')) {
      const index = parseInt(sourceId.replace('career-choice-', ''), 10);
      const newChoices = [...(character.careerSkillChoices ?? [])];
      newChoices[index] = skillName;
      onChange({ careerSkillChoices: newChoices });
    }
  };

  // Count total selections needed vs made
  const selectableSources = skillSources.filter((s) => !s.isGranted);
  const selectionsComplete = selectableSources.filter((s) => s.selectedSkill).length;
    ...
    <div className="h-[500px] flex flex-col">
      <div className="flex-shrink-0">
        ...
        <p className="mb-4 text-sm text-zinc-400">
          Your skills come from your culture and career. Select one skill from each source below.
        </p>
      </div>
      ...
      <ScrollArea className="flex-1">
        <div className="space-y-6 pr-4">
          {skillSources.map((source) => {
            const availableSkills = getAvailableSkillsForSource(source.skillGroups);
              ...
              <Card key={source.id} className="border-zinc-700 bg-zinc-900/30">
                <CardContent className="p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h3 className="font-medium text-zinc-200">{source.label}</h3>
                          ...
                          return (
                            ...
                            >
                              ...
                              {(isSelected || source.isGranted) && (
                                <Check className={cn('h-3.5 w-3.5', isSelected && !source.isGranted && 'text-creator-highlight')} />
                              )}
                            </button>
                          );
                        })}
