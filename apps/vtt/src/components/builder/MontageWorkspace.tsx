...
function normalizeChallenge(raw: unknown): MontageChallenge {
  ...
  const legacySkill =
    typeof item.skill === "string" && item.skill ? [item.skill] : [];
  const legacyCharacteristic =
    typeof item.characteristic === "string" && item.characteristic
      ? [item.characteristic]
      : [];
...
export function MontageWorkspace({
  ...
  const challenges = useMemo<MontageChallenge[]>(() => {
    ...
    if (typeof raw === "string" && raw.trim()) {
      return raw
        .split("\n")
        .filter(Boolean)
        .map((name) => normalizeChallenge({ name: name.trim() }));
    }
    return [];
  }, [data]);
  ...
  const montageData: MontageSceneData = {
    ...
    challenges,
    totalSuccess:
      ((data["totalSuccess"] ?? data["successOutcome"]) as string) ?? "",
    partialSuccess:
      ((data["partialSuccess"] ?? data["partialOutcome"]) as string) ?? "",
    totalFailure:
      ((data["totalFailure"] ?? data["failureOutcome"]) as string) ?? "",
  ...
  const removeChallenge = useCallback(
    (id: string) => {
      updateData({
        challenges: montageData.challenges.filter(
  ...
  const toggleCharacteristic = useCallback(
    (id: string, characteristic: string) => {
      ...
      if (!challenge) return;
      const selected =
        challenge.suggestedCharacteristics.includes(characteristic);
      updateChallenge(id, {
        suggestedCharacteristics: selected
          ? challenge.suggestedCharacteristics.filter(
          ...
          <TabsContent value="setup" className="mt-0">
            <div className="flex flex-col gap-5">
              <SceneAudioPanel
                ...
                onAudioChange={(id) =>
                  updateData({ audioMusic: id ?? undefined })
                }
              />
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="flex flex-col gap-1.5">
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      updateData({ roundLimit: Number(event.target.value) })
                    }
                    className="text-sm"
                  />
                </label>
                <label className="flex flex-col gap-1.5">
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      updateData({ heroCount: Number(event.target.value) })
                    }
                    className="text-sm"
                  />
                </label>
                <label className="flex flex-col gap-1.5">
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      updateData({
                        successesNeeded: Number(event.target.value),
                      })
                    }
                    className="text-sm"
                  />
                ...
                <label className="flex flex-col gap-1.5">
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      updateData({ failureLimit: Number(event.target.value) })
                    }
                    className="text-sm"
                  />
                </label>
                ...
                <div className="flex flex-col gap-3">
                  {montageData.challenges.length === 0 && (
                    <p className="text-xs text-zinc-500">
                      Add obstacles with suggested tests the heroes can attempt.
                    </p>
                  )}
                  {montageData.challenges.map((challenge, index) => (
                    <div
                      key={challenge.id}
                      className="flex flex-col gap-3 rounded-md border border-zinc-700 bg-zinc-900/80 p-3"
                    >
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {CHARACTERISTICS.map((characteristic) => {
                            const active =
                              challenge.suggestedCharacteristics.includes(
                                characteristic,
                              );
                            return (
                              <button
                      ...
                      <label className="flex flex-col gap-1.5">
                        ...
                        <Input
                          ...
                          onChange={(event) =>
                            updateChallenge(challenge.id, {
                              suggestedSkills: splitList(event.target.value),
                            })
                          }
              ...
              <label className="flex flex-col gap-1.5">
                ...
                <textarea
                  ...
                  onChange={(event) =>
                    updateData({
                      totalSuccess: event.target.value,
              ...
              <label className="flex flex-col gap-1.5">
                ...
                <textarea
                  ...
                  onChange={(event) =>
                    updateData({
                      partialSuccess: event.target.value,
                      partialOutcome: event.target.value,
                    })
                  }
              ...
              <label className="flex flex-col gap-1.5">
                ...
                <textarea
                  ...
                  onChange={(event) =>
                    updateData({
                      totalFailure: event.target.value,
