...
const RESPITE_ACTIVITIES = [
  {
    id: "recover",
    label: "Recover",
    description: "Heal stamina and conditions",
  },
  { id: "craft", label: "Craft", description: "Create items or equipment" },
  {
    id: "research",
    label: "Research",
    description: "Study lore or gather information",
  },
  {
    id: "socialize",
    label: "Socialize",
  ...
  { id: "change_kit", label: "Change Kit", description: "Swap equipped kit" },
  {
    id: "project",
    label: "Project",
    description: "Work on long-term projects",
  },
...
export function RespiteWorkspace({
  ...
  const updateField = <K extends keyof RespiteSceneData>(
    field: K,
    value: RespiteSceneData[K],
  ) => {
  ...
  const removeProject = useCallback(
    (id: string) => {
      onChange({
        ...data,
        projects: respiteData.projects.filter((p) => p.id !== id),
      });
    },
    [data, respiteData.projects, onChange],
  );
            ...
            <div className="flex items-center justify-center pb-8">
              <div className="text-center text-zinc-600">
                ...
                <p className="mt-1 text-sm">
                  Set a location and configure activities in the sidebar
                </p>
              </div>
            </div>
          )}
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-2">
                    {RESPITE_ACTIVITIES.map((activity) => {
                      const isSelected =
                        respiteData.availableActivities.includes(activity.id);
                      return (
                        <button
                          key={activity.id}
                          type="button"
