...
/**
 * Active (script-capable) content types must never be served inline. Beyond the
 * explicit allowlist, reject anything whose MIME hints at SVG/XML/HTML/script so
 * near-misses like `image/svg` (without `+xml`) cannot slip past the check.
 */
