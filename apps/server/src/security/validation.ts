...
/**
 * True if any object in the structure carries a prototype-pollution key
 * (`__proto__`, `prototype`, `constructor`). Use before persisting or merging
 * client-supplied JSON. The depth cap bounds traversal cost on hostile input.
 */
...
/**
 * True if the structure nests deeper than `maxDepth` levels of objects/arrays.
 * Guards against deeply-nested JSON that fits a byte budget but can exhaust the
 * stack during later JSON operations.
 */
