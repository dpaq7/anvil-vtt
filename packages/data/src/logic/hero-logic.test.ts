 ...
 * Hero Logic Tests
 *
 * Locks in the Draw Steel derived-vitals rules used wherever stamina is shown:
 * winded threshold (half of max), death threshold (its negative), and recovery
...
describe('HeroLogic derived vitals', () => {
  ...
  describe('getRecoveryValue', () => {
    ...
    it('throws on non-positive max stamina', () => {
      expect(() => getRecoveryValue(0)).toThrow();
    });
  });
});
