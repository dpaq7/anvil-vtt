 ...
 * resolution — which every test and ability resolution depends on.
 */
...
describe('RollLogic.getTier', () => {
  it('returns tier 1 for totals of 11 or less', () => {
  ...
  it('returns tier 3 for totals of 17 or more', () => {
    expect(RollLogic.getTier(17)).toBe(3);
    expect(RollLogic.getTier(25)).toBe(3);
  });

  it('places the 11/12 and 16/17 boundaries exactly', () => {
    expect(RollLogic.getTier(11)).toBe(1);
    expect(RollLogic.getTier(12)).toBe(2);
    expect(RollLogic.getTier(16)).toBe(2);
    expect(RollLogic.getTier(17)).toBe(3);
...
describe('RollLogic edge/bane resolution', () => {
  it('cancels edges and banes one for one', () => {
  ...
  it('applies a flat +/-2 for single edge/bane and none for double', () => {
  ...
  it('shifts tier by +/-1 only for double edge/bane', () => {
    ...
    expect(RollLogic.getTierShift('standard')).toBe(0);
  });

  it('clamps shifted tiers into the 1-3 range', () => {
